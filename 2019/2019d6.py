...
def orbits(li,diag=0):
    ...
    for i in li: 
        ...
        first3 = v[0]
        d[last3] = first3
        if diag: print("key =",last3," value =",first3)
    return d 


def dist(key,dic,centre="COM",distance=1):
    if dic[key]==centre:
        return distance
    else:
        distance += 1
        return dist(dic[key], dic, centre, distance)
        

def path(key,dic,centre="COM",pat=""):
    if dic[key]==centre:
        return key+pat
    else:
        pat=key+pat
        return path(dic[key], dic, centre, pat)
...
def count(li):
    ...
    dic = orbits(li)
    for i in dic:
        counted += dist(i,dic,"COM")
    return counted
    
    
def lcd(pointA,pointB,dic):
    pathA = path(pointA,dic)
    pathB = path(pointB,dic)
    for i in range(min(len(pathA),len(pathB))):
        if pathA[0:3] != pathB[0:3]:
            return (pathA,pathB,common)
        else:
            common = pathA[0:3]
            pathA = pathA[3:]
            pathB = pathB[3:]
...
def traveldist(li): # needs the output of lcd
    Atolcd = len(li[0])/3
    Btolcd = len(li[1])/3
    return int(Atolcd+Btolcd-2)


t1 = ["COM)BBB","BBB)CCC","CCC)DDD","DDD)EEE","EEE)FFF","BBB)GGG","GGG)HHH","DDD)III","EEE)JJJ","JJJ)KKK","KKK)LLL"]
#print(path("DDD",orbits(t1),"COM"))

assert(count(t1)==42)
assert(count(inp)==119831)

t2 = ["COM)BBB","BBB)CCC","CCC)DDD","DDD)EEE","EEE)FFF","BBB)GGG","GGG)HHH","DDD)III","EEE)JJJ","JJJ)KKK","KKK)LLL","KKK)YOU","III)SAN"]
#print(path("YOU",orbits(t2),"COM"))
#print(traveldist(lcd("YOU","SAN",orbits(t2))))

assert(traveldist(lcd("YOU","SAN",orbits(t2)))==4)
print(traveldist(lcd("YOU","SAN",orbits(inp))))
